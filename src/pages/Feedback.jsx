...
const Feedbacks = () => {
  ...
  useEffect(() => {
    const fetchIssues = async () => {
      ...
      try {
        const response = await getIssuesByCreatorIDAction(id);

        if (isMounted && response?.data) {
      // Filter only issues that have at least one feedback
      const filteredIssues = response.data.filter(issue => issue.feedbacks && issue.feedbacks.length > 0);
...
const gridRef = useRef(null); // Using ref to store the GridComponent
  ...
  const toolbarOptions = ['Search', { text: 'Add New Issue', prefixIcon: 'e-add', id: 'addIssue' }];
...
>
  {selectedIssue && (
    <div key={selectedIssue._id} style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <p style={{ margin: '10px 0', lineHeight: '1.5', color: '#555' }}>
        <strong>Category:</strong> {selectedIssue.category.name}
      </p>
      <p style={{ margin: '10px 0', lineHeight: '1.5', color: '#555' }}>
        <strong>Department:</strong> {selectedIssue.department.name}
      </p>

      {/* Feedback section */}
      <div style={{ marginTop: '20px' }}>
        <h3 style={{ color: '#007bff' }}>Feedbacks</h3>
        {selectedIssue.feedbacks.length > 0 ? (
          selectedIssue.feedbacks.map((feedback) => (
            <div key={feedback._id} style={{ background: '#f8f9fa', padding: '10px', borderRadius: '5px', marginBottom: '10px' }}>
              <p style={{ margin: '5px 0', color: '#333' }}>
                <strong>From:</strong> {feedback.author.firstName} {feedback.author.lastName}
              </p>
              <p style={{ margin: '5px 0', color: '#666' }}>
                <strong>Message:</strong> {feedback.content}
              </p>
              <p style={{ margin: '5px 0', fontSize: '12px', color: '#999' }}>
                <strong>Time:</strong> {new Date(feedback.createdAt).toLocaleString()}
              </p>
            </div>
          ))
        ) : (
